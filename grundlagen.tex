...
Zudem ist \ac{SAP IAS} nur als Cloud-Service verfügbar und kann nicht lokal betrieben werden. Im Gegensatz zu \ac{AWS} Cognito, das über die \ac{SDK} eine direkte Verbindung auch aus lokalen Entwicklungsumgebungen ermöglicht, braucht \ac{SAP IAS} ein Anwendung auf der \ac{SAP BTP}, um als \ac{IdP} zu funktionieren.

Zudem unterstützt \ac{SAP IAS} keine \ac{SDK} gesteuerte Benutzerregistrierung. Die Benutzer werden nur u. a. über die Admin-Konsole erstellt. \ac{AWS} Cognito hingegen bietet sowohl Admin-Erstellung als auch SDK-gesteuerte Registrierung an. Für den Vergleich in $H_6$ werden daher die Klicks in beiden Admin-Konsolen gezählt, während die SDK-Registrierung bei \ac{AWS} Cognito als zusätzliches Information dokumentiert wird.
