...
\textbf{Wichtiger Unterschied:} \ac{SAP IAS} ist ausschließlich als Cloud-Service verfügbar und kann nicht lokal betrieben werden. Im Gegensatz zu \ac{AWS} Cognito, das über das \ac{SDK} eine direkte Verbindung auch aus lokalen Entwicklungsumgebungen ermöglicht, erfordert \ac{SAP IAS} ein Deployment auf der \ac{SAP BTP} für die vollständige Funktionalität. Für lokale Entwicklung wird daher ein Mock-Modus im \ac{SAP CAP} Framework verwendet. Dies ist ein wesentlicher architektonischer Unterschied, der bei der Hypothesenprüfung ($H_5$, $H_6$, $H_7$, $H_8$) berücksichtigt wird.

Zudem unterstützt \ac{SAP IAS} \textbf{keine programmatische Benutzerregistrierung} über eine Backend-API. Benutzer werden ausschließlich über die Admin-Konsole oder das Self-Service-Portal erstellt. \ac{AWS} Cognito hingegen bietet sowohl Admin-Erstellung als auch SDK-gesteuerte Registrierung an. Für den Vergleich in $H_6$ werden daher die Klicks in beiden Admin-Konsolen gezählt, während die SDK-Registrierung bei \ac{AWS} Cognito als zusätzliches differenzierendes Feature dokumentiert wird.
