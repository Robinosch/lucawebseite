...
\textbf{H1 – Sicherheit der Autorisierungslogik:} Die Anzahl der Dateien mit Autorisierungslogik wird gezählt, um die Streuung der Sicherheitsregeln zu quantifizieren. Zusätzlich wird die zyklomatische Komplexität der Autorisierungsschicht mittels statischer Code-Analyse gemessen. Die zyklomatische Komplexität (CC) quantifiziert die Anzahl unabhängiger Pfade durch den Code und ist ein etabliertes Maß für die Fehleranfälligkeit von Software \citep{McCabe1976}.
Für \ac{SAP CAP} werden alle \texttt{@restrict}-Annotationen im \ac{CDS}-Modell gezählt.
Da diese rein deklarativ sind, beträgt die zyklomatische Komplexität CC=0.
Für Spring Boot werden alle \texttt{@PreAuthorize}-Annotationen, der \texttt{JwtAuthenticationFilter} und die \texttt{SecurityConfig} analysiert.
...
\begin{itemize}
    \item \textbf{Sicherheitsaspekt:} Gibt es einen Link-basierten oder Code-basierten Flow oder welche anderen Mechanismen werden verwendet?
...
Die Rollen werden einmal als Scopes definiert in der \texttt{xs-security.json}. Abbildung \ref{fig:xs_security} zeigt einen Ausschnitt der definierten Scopes, dabei werden in der gleichen Datei auch Rollensammlungen und Rollenvorlagen definiert.
Diese müssen lediglich die Scopes referenzieren und dadurch in der \ac{SAP BTP} diese den Role Collections zugewiesen.
...
    \end{verbatim}
    \caption{Ausschnitt der Scopes-Definition in \texttt{xs-security.json}}
    \label{fig:xs_security}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/sapbtp_role_collections.png}
    \caption{Screenshot der erstellten Rollensammlungen aus der SAP BTP}
    \label{fig:sapbtp_role_collection}
\end{figure}

Abbildung \ref{fig:sapbtp_role_collection} zeigt die erstellten Rollensammlungen und die Zuweisung der Rollen für die Rollensammlung. Zudem können Benutzer zu den jeweiligen Rollensammlungen zugeordnet werden.
Damit die Rollen auch korrekt von der Anwendung erkannt werden, muss in \ac{SAP IAS} auch Gruppen mit den jeweiligen Rollen erstellt werden. Dadurch wird das Rollem-Mapping zwischen der Anwendung und \ac{SAP IAS} sichergestellt, da der Benutzer sich gegen \ac{SAP IAS} authentifiziert.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/sapias_groups.png}
    \caption{Screenshot der erstellten Gruppen für das Rollemapping in SAP IAS}
    \label{fig:sapias_groups}
\end{figure}

Dabei zeigt Abbildung \ref{fig:sapias_groups} die erstellten Gruppen in \ac{SAP IAS}, die den Rollen im Backend entsprechen. Dabei wird auch die Anwendung referenziert, damit die Gruppen für diese Anwendung gelten und das Mapping korrekt funktioniert.
