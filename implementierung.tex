...
Die Hypothesen sind in zwei Kategorien unterteilt, um einmal die Framework-Integration und auch die \ac{IdP}s selbst zu evaluieren. Die erste Kategorie ($H_1$ bis $H_5$) untersucht die \textbf{Framework-Integration mit den jeweiligen \ac{IdP}s}, also Spring Boot mit \ac{AWS} Cognito und \ac{SAP CAP} mit \ac{SAP IAS} die Authentifizierung und Autorisierung implementieren. Die zweite Kategorie ($H_6$ bis $H_9$) fokussiert sich auf den \textbf{direkten Vergleich der \ac{IdP}s} (\ac{AWS} Cognito vs. \ac{SAP IAS}), unabhängig von der Framework-Implementierung.
...
\begin{enumerate}
    \item[$H_6$] \textbf{IdP -- Benutzer-Onboarding:} Die manuelle Benutzererstellung über die Admin-Konsole erfordert bei \ac{SAP IAS} weniger Klicks/Schritte als bei \ac{AWS} Cognito.
...
Die Hypothesen wurden formuliert, um Unterschiede zwischen den
Frameworks und \ac{IdP}s zu messen und nicht von verschiedenen Entwickler-Kompetenzen abhängig
zu sein. Alle Messungen basieren daher auf objektiven Metriken wie Code-Zeilen,
...
\textbf{H1 – Sicherheit der Autorisierungslogik:} Die Anzahl der Dateien mit Autorisierungslogik wird gezählt, um die Streuung der Sicherheitsregeln zu quantifizieren.
Zusätzlich wird die Anzahl explizit geschriebener Codezeilen
...
\begin{itemize}
    \item \textbf{Sicherheitsaspekt:} Link-basiert (\ac{SAP IAS}) vs. Code-basiert (\ac{AWS} Cognito). Link-basierte Resets gelten als sicherer, da der Code nicht manuell übertragen werden muss.
...
\begin{itemize}
    ...
    \item \textbf{Zusätzlich:} Anzahl der Bestätigungsschritte (Confirmation Dialogs) für die Löschung.
\end{itemize}

\subsubsection{Testumgebungen und Messkontext}

Die Hypothesen zur Framework-Integration ($H_1$ bis $H_5$) messen Framework-inhärente Eigenschaften wie Code-Struktur, Autorisierungslogik und Vendor-Kopplung. Diese werden durch statische Code-Analyse erfasst und sind unabhängig von der Laufzeitumgebung.
...
\begin{itemize}
    ...
    \item \textbf{Recovery-Mechanismus ($H_7$):} Der Passwort-Vergessen-Flow wird qualitativ verglichen (Link-basiert vs. Code-basiert). Für das Spring Boot Backend wurde der Recovery-Flow zusätzlich implementiert.
...
Im Gegensatz zur Spring Security Konfiguration für \ac{AWS} Cognito, ist die Konfiguration für \ac{SAP CAP} minimal. Abbildung \ref{fig:cap_config} zeigt die relevanten Einstellungen.
...
cds:
  security:
    xsuaa:
      enabled: true  # XSUAA/SAP IAS via Service Binding
    \end{verbatim}
    \caption{Minimale Security-Konfiguration in application.yaml für Cloud-Deployment}
    \label{fig:cap_config}
\end{figure}

In der Cloud-Umgebung auf der \ac{SAP BTP} ist lediglich die Aktivierung von XSUAA erforderlich. Die Verbindung zu \ac{SAP IAS} erfolgt automatisch über Service-Binding. Es ist keine manuelle \ac{JWKS}-\ac{URI} Konfiguration oder JWT-Filter-Implementierung notwendig -- das Framework übernimmt die gesamte Token-Validierung automatisch.

\subsection{Event Handler für Geschäftslogik}

Für Geschäftslogik wie das Setzen des \texttt{createdBy}-Feldes werden Event Handler verwendet. Abbildung \ref{fig:cap_handler} zeigt einen Ausschnitt des \texttt{OrderServiceHandler}.
...
public class OrderServiceHandler implements EventHandler {
    ...
    \end{verbatim}
    \caption{Event Handler für Geschäftslogik (OrderServiceHandler.java)}
    \label{fig:cap_handler}
\end{figure}

Der Handler enthält \textbf{keine} Autorisierungslogik -- diese wird vollständig durch die \texttt{@restrict}-Annotationen im \ac{CDS}-Modell durchgesetzt. Der Handler fokussiert sich ausschließlich auf Geschäftslogik.
    ...
    \begin{verbatim}
{
  ...
  "role-templates": [
    ...
    \end{verbatim}
    \caption{Rollen-Definition in xs-security.json}
...
\textbf{Hinweis:} Im Gegensatz zu \ac{AWS} Cognito, bei dem ein User Pool manuell erstellt werden muss (vgl. Abbildung \ref{fig:cognito_user_pool}), erfolgt die \ac{IdP}-Konfiguration bei \ac{SAP IAS} automatisch durch das XSUAA Service-Binding in der \texttt{mta.yaml}.

\subsection{Unterschiede zu AWS Cognito: Benutzerregistrierung und Login}

Ein wesentlicher Unterschied zu \ac{AWS} Cognito liegt im Benutzer-Erstellung:
...
Der Login-Flow in der Cloud erfolgt über OAuth2/OIDC Redirect: Der App Router leitet nicht-authentifizierte Anfragen an \ac{SAP IAS} weiter, wo die Anmeldung erfolgt. Nach erfolgreicher Authentifizierung wird der Benutzer mit einem gültigen Token zurückgeleitet.
...
Der Gesamtumfang der sicherheitsrelevanten Komponenten beträgt circa 30 Zeilen, verteilt auf \texttt{data-model.cds} (20 Zeilen für @restrict) und \texttt{application.yaml} (10 Zeilen Security-Konfiguration). Event Handler wie der \texttt{OrderServiceHandler} enthalten keine Autorisierungslogik. Diese Metriken werden in Kapitel \ref{ch:evaluation} für den Vergleich mit dem Spring Boot Backend herangezogen.
